...
function VoucherCard({ voucher, onCopy, onUse }) {
    ...
    return (
        <div className="group relative flex flex-col overflow-hidden rounded-3xl border border-slate-200/90 bg-white shadow-xs transition-all duration-300 hover:-translate-y-0.5 hover:border-sky-300 hover:shadow-md sm:flex-row">
            ...
            <div
                className={`relative flex flex-col items-center justify-center p-5 text-center sm:w-44 shrink-0 transition-colors ${
                    isUserExclusive
                        ? 'bg-gradient-to-br from-sky-600 to-sky-800 text-white'
                        : isShipping
                            ? 'bg-gradient-to-br from-sky-600 to-blue-700 text-white'
                            : 'bg-gradient-to-br from-sky-50 to-orange-50 text-sky-900 border-b sm:border-b-0 sm:border-r border-dashed border-slate-200'
                }`}
            >
                {/* Icon biểu trưng */}
                ...
                <span className={`text-[10px] font-bold tracking-widest uppercase ${
                    isUserExclusive || isShipping ? 'text-white/80' : 'text-sky-800/70'
                }`}>
                ...
                {isPercentage && voucher.maxDiscountAmount ? (
                    <span className={`mt-1 text-[11px] font-medium leading-tight ${
                        isUserExclusive || isShipping ? 'text-white/90' : 'text-slate-500'
                    }`}>
                ...
                <div className="hidden sm:block absolute -right-3 top-1/2 -translate-y-1/2 h-6 w-6 rounded-full bg-slate-50 border border-slate-200/90 z-10" />
            </div>
            ...
            <div className="flex flex-1 flex-col justify-between p-5">
                <div>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                        <div className="flex flex-wrap items-center gap-2">
                            {/* Mã Code */}
                            <span className="rounded-3xl border border-sky-300 bg-sky-50/90 px-2.5 py-1 font-mono text-xs font-extrabold text-sky-900 tracking-wider">
                                {voucher.code}
                            </span>
                            ...
                            {/* Loại áp dụng */}
                            <span className="rounded-full bg-sky-100/60 px-2 py-0.5 text-[11px] font-semibold text-sky-900">
                        ...
                        {isUserExclusive && voucher.remainingUsage != null && (
                            <span className="rounded-full bg-sky-50 border border-sky-200 px-2 py-0.5 text-[11px] font-bold text-sky-800">
                    ...
                    {/* Điều kiện & HSD */}
                    <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-slate-500 border-t border-slate-100 pt-2.5">
                        ...
                        <span className="inline-flex items-center gap-1">
                            ...
                            <span>HSD: <strong className="text-slate-700 font-semibold">{formatDate(voucher.endDate)}</strong></span>
                        </span>
                    </div>
                </div>
                ...
                <div className="mt-4 flex items-center justify-end gap-2.5 pt-2">
                    <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-3xl border border-slate-200 bg-white px-3.5 py-1.5 text-xs font-bold text-slate-700 shadow-2xs hover:bg-slate-50 hover:border-slate-300 transition-colors"
                    ...
                    <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-3xl bg-sky-800 px-4 py-1.5 text-xs font-bold text-white shadow-2xs hover:bg-sky-900 transition-colors"
                    >
                        <span>Dùng ngay</span>
                        <span>→</span>
                    </button>
    ...
    return (
        <div className="mx-auto max-w-3xl px-4 py-8">
            ...
            <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
                ...
                <LoadingLink
                    to="/"
                    className="inline-flex items-center gap-1.5 rounded-3xl bg-slate-100 px-3.5 py-2 text-xs font-bold text-slate-700 hover:bg-slate-200 transition-colors"
                >
            ...
            {!loading && vouchers.length > 0 && (
                <div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-gradient-to-r from-sky-50 to-orange-50 border border-sky-200/80 p-4">
                    <div className="flex items-center gap-3">
                        <span className="flex h-10 w-10 items-center justify-center rounded-3xl bg-sky-800 text-lg text-white shadow-2xs">
                            🎟️
                        </span>
                        <div>
                            <p className="text-xs font-bold text-sky-950">
                                Bạn đang có <strong className="text-sky-800 text-sm">{vouchers.length}</strong> mã ưu đãi khả dụng
                            </p>
                            {countPersonal > 0 && (
                    ...
                    <button
                        ...
                        className="rounded-3xl bg-sky-800 px-3.5 py-1.5 text-xs font-bold text-white shadow-2xs hover:bg-sky-900 transition-colors"
                    >
            ...
            <div className="mb-6 space-y-3">
                ...
                <div className="relative">
                    <input
                        ...
                        className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-2.5 pl-10 text-xs text-slate-800 placeholder-slate-400 shadow-2xs focus:border-sky-600 focus:outline-hidden"
                    />
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                    {FILTERS.map((f) => (
                        <button
                            ...
                            className={`rounded-3xl px-3.5 py-1.5 text-xs font-bold transition-all shadow-2xs ${
                                filter === f.key
            ...
            {loading && (
                <div className="flex flex-col gap-4">
                    {[1, 2, 3].map((i) => (
                        <div key={i} className="h-36 animate-pulse rounded-3xl bg-slate-100 border border-slate-200" />
                    ))}
                </div>
            )}
            ...
            {!loading && !error && filteredVouchers.length === 0 && (
                <div className="flex flex-col items-center gap-3 rounded-3xl border border-slate-200 bg-white py-16 text-center shadow-xs">
                    ...
                    <LoadingLink
                        to="/"
                        className="mt-2 inline-flex items-center gap-2 rounded-3xl bg-sky-800 px-5 py-2.5 text-xs font-bold text-white shadow-sm hover:bg-sky-900 transition-colors"
                    >
