            ...
            {[1, 2, 3, 4, 5].map((star) => {
                ...
                return (
                    <svg
                        ...
                        viewBox="0 0 20 20"
                        fill="currentColor"
                        className={`${starClass} ${filled ? 'text-amber-400' : 'text-stone-200'}`}
                    >
                        <path
                            fillRule="evenodd"
...
function RatingProgressBar({ starLabel, count = 0, total = 0 }) {
    ...
    return (
        <div className="flex items-center gap-2.5 text-xs text-stone-600">
            <span className="w-10 shrink-0 font-medium text-stone-500">{starLabel}</span>
            ...
            <span className="w-14 shrink-0 text-right font-mono text-[11px] text-stone-400">
...
export default function ProductFeedbackSection({ productId, summary: initialSummary }) {
    ...
    return (
        <section id="product-reviews" className="mt-12 border-t border-stone-200/80 pt-10">
            {/* Header Tiêu Đề */}
            ...
            <div className="mb-8 rounded-3xl border border-stone-200/80 bg-stone-50/60 p-6 shadow-2xs">
                <div className="grid gap-6 md:grid-cols-12 md:items-center">
                    {/* Cột trái: Điểm trung bình to */}
                    <div className="flex flex-col items-center justify-center border-b border-stone-200/70 pb-5 text-center md:col-span-4 md:border-b-0 md:border-r md:pb-0 md:pr-6">
            ...
            <div id="product-feedback-list" className="space-y-4">
                ...
                {!loading && feedbacks.length === 0 && (
                    <div className="rounded-2xl border border-dashed border-stone-200 bg-white py-12 text-center">
                        ...
                        <p className="mt-2 text-sm font-bold text-stone-700">Chưa có bình luận nào cho món này</p>
                    ...
                    <div className="space-y-3">
                        {feedbacks.map((fb) => {
                            ...
                            return (
                                <div
                                    key={fb.feedbackId}
                                    className="rounded-2xl border border-stone-100 bg-white p-4 shadow-2xs transition hover:border-stone-200"
                                >
                                    {/* Header của feedback: Avatar + Tên + Ngày + Sao */}
                                    <div className="flex items-start justify-between gap-3">
                                        <div className="flex items-center gap-3">
                                            {/* Avatar khách */}
                                            <div className="h-10 w-10 shrink-0 overflow-hidden rounded-full border border-stone-200 bg-linear-to-br from-amber-700 to-stone-800 text-white flex items-center justify-center font-bold text-sm">
                                                {fb.userAvatar ? (
                                                    <img
                                                        src={fb.userAvatar}
                                    ...
                                    <div className="mt-3 pl-13">
                                        {hasComment ? (
                                            <p className="rounded-xl bg-stone-50/80 p-3 text-xs leading-relaxed text-stone-700 border border-stone-100/60">
                                                {fb.comment}
                                            </p>
                                        ) : (
                ...
                {!loading && totalPages > 1 && (
                    <div className="mt-6 flex items-center justify-center gap-1.5 pt-2">
                        <button
                            ...
                            className="h-8 w-8 rounded-xl border border-stone-200 bg-white text-xs font-bold text-stone-600 shadow-2xs hover:bg-stone-50 disabled:opacity-40 disabled:cursor-not-allowed transition"
                        >
                            ‹
                        </button>
